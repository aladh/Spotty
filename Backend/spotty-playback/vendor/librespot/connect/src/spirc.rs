...
struct ObservedTransfer {
    ...
    restored: oneshot::Sender<()>,
}

impl std::fmt::Debug for ObservedTransfer {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("ObservedTransfer")
...
struct SpircTask {
    ...
    // Poll the outbound self-transfer alongside Dealer intake: awaiting it in handle_command
    // prevents this same task from acknowledging the inbound transfer request.
    pending_transfer: Option<Pin<Box<dyn Future<Output = Result<(), Error>> + Send>>>,
    transfer_snapshot: Option<ObservedTransfer>,
    transfer_restored: Option<oneshot::Sender<()>>,
...
enum SpircCommand {
    ...
    SetVolume(u16),
    Activate,
    Transfer(Option<TransferRequest>, Option<Box<ObservedTransfer>>),
    Load(LoadRequest),
...
impl Spirc {
    ...
    pub fn transfer(&self, transfer_request: Option<TransferRequest>) -> Result<(), Error> {
        Ok(self
            .commands
            .send(SpircCommand::Transfer(transfer_request, None))?)
    }

    /// Transfers a validated retained session without replacing its observed queue with a
    /// newly resolved context. The caller must still wait for protocol/player confirmation.
    pub fn transfer_observed(
        &self,
        transfer_request: TransferRequest,
        snapshot: PlayerState,
    ) -> Result<oneshot::Receiver<()>, Error> {
        let (restored, receiver) = oneshot::channel();
        self.commands.send(SpircCommand::Transfer(
            Some(transfer_request),
            Some(Box::new(ObservedTransfer { snapshot, restored })),
        ))?;
        Ok(receiver)
    }
}
...
impl SpircTask {
    async fn run(mut self) {
        ...
        while !self.session.is_invalid() && !self.shutdown {
            ...
            tokio::select! {
                ...
                result = async { self.pending_transfer.as_mut().expect("guarded transfer").await }, if self.pending_transfer.is_some() => {
                    self.pending_transfer = None;
                    if let Err(why) = result {
                        self.transfer_snapshot = None;
                        self.transfer_restored = None;
                        error!("transfer request failed: {why}");
                    }
                },
    ...
    fn handle_next_context(&mut self, next_context: Result<Context, Error>) -> bool {
        ...
        if update_state {
            ...
            if self.transfer_state.is_none() {
                if let Some(restored) = self.transfer_restored.take() {
                    let _ = restored.send(());
                }
            }
        }
    ...
    async fn handle_command(&mut self, cmd: SpircCommand) -> Result<(), Error> {
        ...
        match cmd {
            ...
            SpircCommand::Transfer(request, snapshot) if !self.connect_state.is_active() => {
                if self.pending_transfer.is_some() {
                    return Err(Error::failed_precondition("transfer already pending"));
                }
                self.transfer_snapshot = snapshot.map(|state| *state);
                let session = self.session.clone();
                self.pending_transfer = Some(Box::pin(async move {
                    ...
                    .await
                    .map_err(|_| Error::deadline_exceeded("transfer confirmation expired"))?
                    .map(|_| ())
                }));
                return Ok(());
            }
    ...
    async fn handle_connection_id_update(&mut self, connection_id: String) -> Result<(), Error> {
        ...
        match TransferState::parse_from_bytes(&cluster.transfer_data) {
            Ok(transfer_state) => self.handle_transfer(transfer_state, None)?,
        ...
        if let Some(cluster) = cluster_update.cluster.take() {
            ...
            if became_inactive {
                ...
                && cluster_update
                    ...
                    .iter()
                    .any(|id| id != self.session.device_id())
            {
                // Echoing our own state creates a notify/update loop that starves context
                // restoration and can exhaust Spotify's request limit.
    ...
    fn take_observed_transfer(
        ...
        if let Some(observed) = observed.as_ref() {
            ...
            let context = transfer
                ...
                .as_deref()
                .unwrap_or_default();
            if observed.restored.is_closed()
                || !transfer.playback.is_paused()
                || snapshot.track.uri != track.uri
                || snapshot.context_uri != context
                || snapshot.position_as_of_timestamp.abs_diff(i64::from(
                    transfer
                        .playback
    ...
    fn handle_transfer(
        ...
        if let Some(observed) = observed {
            state.begin_observed_transfer(observed.snapshot);
            self.transfer_restored = Some(observed.restored);
        }
        ...
        // Activation emits shuffle/repeat events, so install transferred options first.
        self.handle_activate();
    ...
    fn cancel_observed_transfer(&mut self) {
        self.pending_transfer = None;
        self.transfer_snapshot = None;
        self.transfer_restored = None;
        self.transfer_state = None;
        self.connect_state.cancel_observed_transfer();
    }

    async fn handle_disconnect(&mut self) -> Result<(), Error> {
        self.cancel_observed_transfer();
        self.context_resolver.clear();
