...
impl ConnectState {
    ...
    pub fn update_position_in_relation(&mut self, timestamp: i64) {
        ...
        // Disconnect also calls this for a paused session. Time spent paused must not become
        // played time in the retained state that the next client observes.
        let diff = if is_playing {
...
#[cfg(test)]
#[path = "spotty_transfer_tests.rs"]
mod spotty_transfer_tests;
