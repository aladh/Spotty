...
impl ConnectState {
    ...
    /// Keeps the exact observed occurrence order while background context resolution finishes.
    pub fn begin_observed_transfer(&mut self, snapshot: PlayerState) {
        self.apply_observed_transfer(&snapshot);
        self.observed_transfer = true;
    }

    pub fn cancel_observed_transfer(&mut self) {
        self.observed_transfer = false;
    }

    fn apply_observed_transfer(&mut self, snapshot: &PlayerState) {
        let player = self.player_mut();
        player.track = snapshot.track.clone();
        player.prev_tracks = snapshot.prev_tracks.clone();
        player.next_tracks = snapshot.next_tracks.clone();
        player.options = snapshot.options.clone();
        player.index = snapshot.index.clone();
        player.duration = snapshot.duration;
    ...
    pub fn finish_transfer(&mut self, transfer: TransferState) -> Result<(), Error> {
        ...
        if self.observed_transfer {
            // The protocol already supplied the current occurrence and ordered queue. Context
            // resolution only establishes where later refill starts; it must not reconstruct
            // this session from a changed playlist or a different shuffle algorithm.
            ...
            let tail = self
                ...
                .iter()
                .rev()
                .find(|track| track.is_context())
                .or(self.player().track.as_ref());
            let next_index = tail
                .and_then(|tail| {
                    ctx.tracks.iter().position(|track| {
                        if !tail.uid.is_empty() {
                            track.uid == tail.uid
                        } else {
                            track.uri == tail.uri
                        }
                ...
                .map(|index| index + 1)
                .unwrap_or(ctx.tracks.len());
            self.update_context_index(self.fill_up_context, next_index)?;
            self.observed_transfer = false;
