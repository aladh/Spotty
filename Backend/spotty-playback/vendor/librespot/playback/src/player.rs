...
fn stop_for_key_refusal(key_refused: bool, decoder_failed: bool) -> bool {
    key_refused && decoder_failed
}

fn explicitly_refused_key(error: &Error) -> bool {
    matches!(
        error
            .error
...
pub enum PlayerEvent {
    ...
    /// Explicit audio-key refusal followed by decoder failure. Does not advance the queue.
    AudioKeyRefused {
        play_request_id: u64,
        track_id: SpotifyUri,
    },
...
enum PlayerPreload {
    ...
    Loading {
        track_id: SpotifyUri,
        loader:
            Pin<Box<dyn FusedFuture<Output = Result<PlayerLoadedTrackData, LoadFailure>> + Send>>,
    },
    Ready {
        track_id: SpotifyUri,
...
struct PlayerTrackLoader {
    key_refused_decode_failure: std::sync::atomic::AtomicBool,
    session: Session,
    config: PlayerConfig,
    local_file_lookup: Arc<LocalFileLookup>,
}
    ...
    async fn load_remote_track(
        ...
        loop {
            ...
            let key = match self.session.audio_key().request(track_id, file_id).await {
                Ok(key) => Some(key),
                Err(e) => {
                    key_refused = explicitly_refused_key(&e);
            ...
            let mut decoder = match decoder_type {
                Ok(decoder) => decoder,
                Err(_) if stop_for_key_refusal(key_refused, true) => {
                    self.key_refused_decode_failure
        ...
        loop {
            ...
            } = self.state
            {
                ...
                if !loader.as_mut().is_terminated() {
                    match loader.as_mut().poll(cx) {
                        ...
                        Poll::Ready(Err(LoadFailure::AudioKeyRefused)) => {
                            self.send_event(PlayerEvent::AudioKeyRefused {
                                track_id,
                                play_request_id,
                            });
                            // Stopped is already understood by Spirc and preserves queue occurrences.
            ...
            } = self.preload
            {
                ...
                match loader.as_mut().poll(cx) {
                    ...
                    Poll::Ready(Err(LoadFailure::AudioKeyRefused)) => {
                        // A refusal must not mark an upcoming occurrence unavailable.
...
impl PlayerInternal {
    ...
    ) -> impl FusedFuture<Output = Result<PlayerLoadedTrackData, LoadFailure>> + Send + 'static
    {
        ...
        let loader = PlayerTrackLoader {
            key_refused_decode_failure: std::sync::atomic::AtomicBool::new(false),
        ...
        let load_handle = thread::spawn(move || {
            ...
            let failure = if loader.key_refused_decode_failure.load(Ordering::Relaxed) {
                LoadFailure::AudioKeyRefused
            } else {
                LoadFailure::Unavailable
            };
            let _ = result_tx.send(data.ok_or(failure));
        ...
        result_rx
            .map(|result| result.unwrap_or(Err(LoadFailure::Unavailable)))
            .fuse()
    }
...
#[cfg(test)]
#[path = "spotty_player_tests.rs"]
mod spotty_key_refusal_tests;
