...
nonisolated enum CatalogMapping {
    ...
    static func playlistTrack(from entry: PathfinderPlaylistItem) -> CatalogTrack? {
        ...
        let uid = entry.uid.flatMap {
            !$0.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty && $0 != uri ? $0 : nil
        }
        return CatalogTrack(
            id: uid ?? uri,
            uri: uri,
    ...
    /// Resolve identity only after the entire playlist has been read. Duplicate server UIDs
    /// are ambiguous even when one duplicate has unsupported track content.
    static func playlistTracks(from entries: [PathfinderPlaylistItem]) -> [CatalogTrack] {
        let uidCounts = Dictionary(entries.compactMap(\.uid).map { ($0, 1) }, uniquingKeysWith: +)
        let tracks = entries.compactMap(playlistTrack(from:))
        let reservedIDs = Set(
            tracks.compactMap { track in
                track.occurrenceUID.flatMap { uidCounts[$0] == 1 ? $0 : nil }
            })
        var ordinals: [String: Int] = [:]
        var usedIDs = reservedIDs
        return tracks.map { track in
            let ordinal = ordinals[track.uri, default: 0]
            ordinals[track.uri] = ordinal + 1
            let uid = track.occurrenceUID.flatMap { uidCounts[$0] == 1 ? $0 : nil }
            let displayID: String
            if let uid {
                displayID = uid
            } else {
                let base = "catalog:\(track.uri.utf8.count):\(track.uri):\(ordinal)"
                var candidate = base
                var collision = 0
