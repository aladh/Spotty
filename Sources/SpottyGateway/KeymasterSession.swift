...
actor KeymasterSession {
    ...
    func accessToken(expectedGeneration: Int) async throws -> String {
        guard generation == expectedGeneration else { throw KeymasterSessionError.noGrant }
        let token = try await accessToken()
        guard generation == expectedGeneration else { throw KeymasterSessionError.noGrant }
        return token
    }

    func refreshIgnoringExpiry(rejected: String, expectedGeneration: Int) async throws -> String {
        guard generation == expectedGeneration else { throw KeymasterSessionError.noGrant }
        let token = try await refreshIgnoringExpiry(rejected: rejected)
        guard generation == expectedGeneration else { throw KeymasterSessionError.noGrant }
        return token
    }
