...
nonisolated struct SpotifyConnectWireCommand: Encodable, Sendable {
    ...
    static let pause = SpotifyConnectWireCommand(endpoint: .pause)
    static let resume = SpotifyConnectWireCommand(endpoint: .resume)
    static let next = SpotifyConnectWireCommand(endpoint: .next)
    static let previous = SpotifyConnectWireCommand(endpoint: .previous)

    static func seek(to milliseconds: Int) -> Self {
        SpotifyConnectWireCommand(endpoint: .seek, value: .integer(max(0, milliseconds)))
    }

    static func shuffle(_ enabled: Bool) -> Self {
        SpotifyConnectWireCommand(endpoint: .shuffle, value: .boolean(enabled))
    }

    static func repeatContext(_ enabled: Bool) -> Self {
        SpotifyConnectWireCommand(endpoint: .repeatContext, value: .boolean(enabled))
    }

    static func repeatTrack(_ enabled: Bool) -> Self {
        SpotifyConnectWireCommand(endpoint: .repeatTrack, value: .boolean(enabled))
    }
    ...
    static func addToQueue(_ uri: String) -> Self {
        SpotifyConnectWireCommand(endpoint: .addToQueue, track: Track(uri: uri))
    }
    ...
    static func setQueue(
        ...
        var command = SpotifyConnectWireCommand(endpoint: .setQueue)
    ...
    static func play(uri: String, trackIndex: Int? = nil) -> Self {
        SpotifyConnectWireCommand(endpoint: .play, context: Context(uri: uri, trackIndex: trackIndex))
    }

    static func play(trackURIs: [String]) -> Self {
        SpotifyConnectWireCommand(endpoint: .play, context: Context(trackURIs: trackURIs))
    }
}

extension SpotifyConnectWireCommand {
    init(_ command: SpotifyConnectCommand) {
        let endpoint: Kind
        switch command.endpoint {
        case .pause: endpoint = .pause
        case .resume: endpoint = .resume
        case .next: endpoint = .next
        case .previous: endpoint = .previous
        case .seek: endpoint = .seek
        case .shuffle: endpoint = .shuffle
        case .repeatContext: endpoint = .repeatContext
        case .repeatTrack: endpoint = .repeatTrack
        case .addToQueue: endpoint = .addToQueue
        case .setQueue: endpoint = .setQueue
        case .play: endpoint = .play
        }
        let value: Value? = command.value.map {
            switch $0 {
            case let .integer(value): .integer(value)
            case let .boolean(value): .boolean(value)
            }
        }
        let context: Context? = command.context.map {
            if let uris = $0.trackURIs { return Context(trackURIs: uris) }
            return Context(uri: $0.uri, trackIndex: $0.trackIndex)
        }
        self.init(
            endpoint: endpoint,
            value: value,
            context: context,
            track: command.track.map { Track(uri: $0.uri) },
            nextTracks: command.nextTracks?.map(QueueTrack.init),
            prevTracks: command.prevTracks?.map(QueueTrack.init),
            queueRevision: command.queueRevision
        )
    }
}
