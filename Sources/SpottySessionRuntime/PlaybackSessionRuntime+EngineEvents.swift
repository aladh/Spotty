...
//
//  PlaybackSessionRuntime+EngineEvents.swift
//  Spotty
//
...
extension PlaybackSessionRuntime {
    ...
    /// SessionRuntimeActor turn. Original source revisions/receipt times remain authoritative;
    /// no network command or engine rebuild is needed to recover from a slow runtime consumer.
    ...
    /// SessionRuntimeActor dedupe for Connect queue *callbacks*. Records generation and revision together
    ...
    /// a load whose session or window is gone; the coordinator's SessionRuntimeActor re-check is only an
    /// early-out.
