...
struct PlaybackDispatchRoutingTests {
    ...
    @Test
    @MainActor
    func aSameRouteRemoteCommandSurvivesATimingPublication() async {
        ...
        let player = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: engine, remote: remote)
        )
        seedLocalPaused(player)
        player.toggleShuffle()
        #expect(await waitUntil { gate.enteredCount == 1 }, "a local command occupies the coordinator")
        ...
        gate.finish(with: .ok)
        #expect(
            await waitUntil { remote.sendCount == 1 },
    ...
    @Test
    @MainActor
    func aRemoteCommandIsDroppedWhenOwnershipHandsOff() async {
        let engine = HarnessEngine()
        let gate = HarnessEngineGate(result: .ok)
        engine.onExecute = { [gate] _ in gate.enter() }
        let remote = HarnessRemote(send: .succeed)
        ...
        #expect((remote.sendCount) == 0, "handoff causes zero sends to the stale target")
    ...
    @Test
    @MainActor
    func aRemoteCommandIsDroppedOnEngineGenerationCancellation() async {
        let engine = HarnessEngine()
        let gate = HarnessEngineGate(result: .ok)
        engine.onExecute = { [gate] _ in gate.enter() }
        let remote = HarnessRemote(send: .succeed)
        ...
        #expect((remote.sendCount) == 0, "generation cancellation causes zero stale sends")
        await player.shutdownForTermination()
    }

    @Test
    @MainActor
    func aRemoteCommandIsDroppedOnAccountCancellation() async {
        let engine = HarnessEngine()
        let gate = HarnessEngineGate(result: .ok)
        engine.onExecute = { [gate] _ in gate.enter() }
        let remote = HarnessRemote(send: .succeed)
        ...
        #expect((remote.sendCount) == 0, "account cancellation causes zero stale sends")
        await player.shutdownForTermination()
    }

    @Test
    @MainActor
    func firstIdleLocalPlayKeepsItsChosenTrackTarget() async {
        let local = HarnessEngine(executeResult: .ok)
        let player = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: local, remote: HarnessRemote(send: .succeed))
        )
    ...
    @Test
    @MainActor
    func explicitTransferKeepsItsDeliberateDeviceTarget() async {
        let local = HarnessEngine(executeResult: .ok)
        let player = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: local, remote: HarnessRemote(send: .succeed))
        )
...
struct PlaybackCommandFailureTests {
    @Test
    @MainActor
    func engineResultsMapToTypedCommandOutcomes() async {
    ...
    @Test
    @MainActor
    func theCoordinatorReportsLocalCommandOutcomes() async {
        ...
        let reconnectCoordinator = PlaybackCoordinator(
            local: HarnessEngine(executeResult: PlaybackEngineResult(rawValue: -2)),
            remote: HarnessRemote(send: .succeed)
        )
    ...
    @Test
    @MainActor
    func theCoordinatorReportsRemoteCommandOutcomes() async {
        ...
        let sleepingRemote = HarnessRemote(send: .sleepUntilCancelled)
        ...
        let sendStarted = await waitUntil { sleepingRemote.sendCount == 1 }
    ...
    @Test
    @MainActor
    func localPauseFailuresPresentTheirActionNotice() async {
        let action = "Pause was rejected"

        @MainActor
        func runLocal(
            _ result: PlaybackEngineResult,
            account: HarnessAccount = HarnessAccount()
        ) async -> (
            completions: [Bool],
            notice: String?,
            authorizeCount: Int,
            ...
            let player = HarnessEnvironment.makePlaybackStore(
                HarnessEnvironment.make(
                    engine: HarnessEngine(executeResult: result),
                    remote: HarnessRemote(send: .succeed),
                    account: account
                )
            )
        ...
        let readyAccount = HarnessAccount(hasGrant: true, authorization: .succeed)
        let readyEngine = HarnessEngine(executeResult: PlaybackEngineResult(rawValue: -2))
        let ready = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(
                engine: readyEngine,
                remote: HarnessRemote(send: .succeed),
        ...
        let cancelledAccount = HarnessAccount(hasGrant: true, authorization: .succeed)
        let cancelledEngine = HarnessEngine(executeResult: .ok)
        ...
        let reconciledAccount = HarnessAccount(hasGrant: true, authorization: .succeed)
        let reconciledEngine = HarnessEngine()
        let reconciledGate = HarnessEngineGate()
        reconciledEngine.onExecute = { [reconciledGate] _ in reconciledGate.enter() }
        let reconciled = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(
        ...
        #expect(
            (await waitUntil { reconciledGate.enteredCount == 1 }) == true, "the gated engine call is in flight")
        _ = reconciled.send(
            .presentation(
                PlaybackPresentationSnapshot(
                    currentTrack: CurrentTrack(
    ...
    @Test
    @MainActor
    func remotePauseFailuresPresentTheirActionNotice() async {
        let action = "Pause was rejected"

        @MainActor
        func prepareRemoteStore(remote: HarnessRemote) -> PlaybackStore {
            let player = HarnessEnvironment.makePlaybackStore(
                HarnessEnvironment.make(
                    engine: HarnessEngine(executeResult: .ok),
                    remote: remote
        ...
        let rejecting = HarnessRemote(send: .fail)
        ...
        let sleeping = HarnessRemote(send: .sleepUntilCancelled)
    ...
    @Test
    @MainActor
    func pauseAndResumeRollBackTimingWhenTheCommandIsRejected() async {
        ...
        let pauseOkStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(
                engine: HarnessEngine(executeResult: .ok), remote: HarnessRemote(send: .succeed)
            )
        )
        ...
        let seekOkStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(
                engine: HarnessEngine(executeResult: .ok), remote: HarnessRemote(send: .succeed)
            )
        )
        ...
        let duplicateStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: HarnessEngine(executeResult: .ok), remote: duplicateRemote)
        )
        ...
        let cancelRemote = HarnessRemote(send: .sleepUntilCancelled)
        let cancelStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: HarnessEngine(executeResult: .ok), remote: cancelRemote)
        )
        ...
        let pauseReached = await waitUntil { cancelRemote.sendCount == 1 }
        ...
        let staleStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(
                engine: HarnessEngine(executeResult: .ok), remote: HarnessRemote(send: .sleepUntilCancelled))
        )
        ...
        let trackSwitchRemote = HarnessRemote(send: .park)
        let trackSwitchStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: HarnessEngine(executeResult: .ok), remote: trackSwitchRemote)
        )
    ...
    @Test
    @MainActor
    func playCommandsRollBackTheirTrackWhenRejected() async {
        let clockNow = Date(timeIntervalSince1970: 1_800_000_000)
        let playingAnchor = clockNow.addingTimeInterval(-10)
        let priorPlayingTiming = PlaybackTiming(position: 40, duration: 200, anchoredAt: playingAnchor)
        ...
        let localAccepted = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(
                engine: HarnessEngine(executeResult: .ok), remote: HarnessRemote(send: .succeed)
            )
        )
        ...
        let remoteAccepted = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(
                engine: HarnessEngine(executeResult: .ok), remote: HarnessRemote(send: .succeed)
            )
        )
        ...
        let laggingStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: HarnessEngine(executeResult: .ok), remote: laggingRemote)
        )
        ...
        let confirmRemote = HarnessRemote(send: .park)
        let confirmStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: HarnessEngine(executeResult: .ok), remote: confirmRemote)
        )
        ...
        let supersedeRemote = HarnessRemote(send: .park)
        let supersedeStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: HarnessEngine(executeResult: .ok), remote: supersedeRemote)
        )
        ...
        let nilRemote = HarnessRemote(send: .park)
        let nilStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: HarnessEngine(executeResult: .ok), remote: nilRemote)
        )
        ...
        let playReached = await waitUntil { cancelRemote.sendCount == 1 }
        ...
        let unknownPlaylist = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(
                engine: HarnessEngine(executeResult: .ok), remote: HarnessRemote(send: .succeed)
            )
        )
        ...
        let localGate = HarnessEngineGate()
        localEngine.onExecute = { [localGate] _ in localGate.enter() }
        let localRace = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: localEngine, remote: HarnessRemote(send: .succeed))
        )
    ...
    @Test
    @MainActor
    func shuffleCommandsRollBackTheirFlagWhenRejected() async {
        let clockNow = Date(timeIntervalSince1970: 1_800_000_000)
        ...
        func restoredStore(
            ...
            let player = HarnessEnvironment.makePlaybackStore(
                HarnessEnvironment.make(engine: local, remote: remote, preferences: preferences)
            )
        ...
        #expect(
            (localRejectedPrefs.shuffleWrites.isEmpty) == true, "local shuffle rejection does not persist off"
        )
        await localRejected.shutdownForTermination()
        ...
        let remoteAcceptedPrefs = HarnessPreferences(shuffle: true, shuffleHistory: ["restored": 1])
        ...
        let laggingRemote = HarnessRemote(send: .park)
        let laggingPrefs = HarnessPreferences(shuffle: true, shuffleHistory: ["restored": 1])
        ...
        let confirmRemote = HarnessRemote(send: .park)
        let confirmPrefs = HarnessPreferences(shuffle: true, shuffleHistory: ["restored": 1])
        ...
        let localGate = HarnessEngineGate()
        localEngine.onExecute = { [localGate] _ in localGate.enter() }
        let localRacePrefs = HarnessPreferences(shuffle: true, shuffleHistory: ["restored": 1])
        ...
        let joiningPrefs = HarnessPreferences(shuffle: true, shuffleHistory: ["restored": 1])
        ...
        #expect((joiningPrefs.shuffleWrites.isEmpty) == true, "route refusal does not persist shuffle")
        await joining.shutdownForTermination()

        let duplicateRemote = HarnessRemote(send: .sleepUntilCancelled)
        let duplicatePrefs = HarnessPreferences(shuffle: true, shuffleHistory: ["restored": 1])
        ...
        #expect((duplicatePrefs.shuffleWrites.isEmpty) == true, "a duplicate shuffle does not persist")
        ...
        let cancelRemote = HarnessRemote(send: .sleepUntilCancelled)
        let cancelPrefs = HarnessPreferences(shuffle: true, shuffleHistory: ["restored": 1])
        ...
        let shuffleReached = await waitUntil { cancelRemote.sendCount == 1 }
        ...
        #expect((cancelPrefs.shuffleWrites.isEmpty) == true, "cancellation does not persist shuffle")
        await cancelStore.shutdownForTermination()

        let stalePrefs = HarnessPreferences(shuffle: true, shuffleHistory: ["restored": 1])
        let staleStore = await restoredStore(
            local: HarnessEngine(executeResult: .ok),
            remote: HarnessRemote(send: .sleepUntilCancelled),
        ...
        #expect((stalePrefs.shuffleWrites.isEmpty) == true, "an engine-epoch bump does not persist shuffle")
        await staleStore.shutdownForTermination()

        let restoreRemote = HarnessRemote(send: .park)
        let restorePrefs = HarnessPreferences(shuffle: true, shuffleHistory: ["restored": 1])
        ...
        let matchingRemote = HarnessRemote(send: .park)
        let matchingPrefs = HarnessPreferences(shuffle: true, shuffleHistory: ["restored": 1])
        ...
        #expect(
            (matchingPrefs.shuffleWrites.isEmpty) == true,
            "rejection after only a matching user options event does not persist off")
        await matchingStore.shutdownForTermination()

        let persistEngine = HarnessEngine()
        let persistGate = HarnessEngineGate()
        persistEngine.onExecute = { [persistGate] _ in persistGate.enter() }
        let persistPrefs = HarnessPreferences(shuffle: true, shuffleHistory: ["restored": 1])
        ...
        let preferenceOnlyPrefs = HarnessPreferences(shuffle: false, shuffleHistory: ["restored": 1])
    ...
    @Test
    @MainActor
    func transferCommandsRollBackOwnershipWhenRejected() async {
        let clockNow = Date(timeIntervalSince1970: 1_800_000_000)
        let priorPlayingTiming = PlaybackTiming(
            position: 40, duration: 200, anchoredAt: clockNow.addingTimeInterval(-10))
        let current = CurrentTrack(
        ...
        let localAcceptedEngine = HarnessEngine(executeResult: .ok)
        let localAccepted = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(
                engine: localAcceptedEngine,
                remote: HarnessRemote(send: .succeed)
            )
        )
        ...
        let laggingEngine = HarnessEngine()
        let laggingGate = HarnessEngineGate()
        laggingEngine.onExecute = { [laggingGate] _ in laggingGate.enter() }
        let laggingStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: laggingEngine, remote: HarnessRemote(send: .succeed))
        )
        ...
        let confirmEngine = HarnessEngine()
        let confirmGate = HarnessEngineGate()
        confirmEngine.onExecute = { [confirmGate] _ in confirmGate.enter() }
        let confirmStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: confirmEngine, remote: HarnessRemote(send: .succeed))
        )
        ...
        let supersedeEngine = HarnessEngine()
        let supersedeGate = HarnessEngineGate()
        supersedeEngine.onExecute = { [supersedeGate] _ in supersedeGate.enter() }
        let supersedeStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: supersedeEngine, remote: HarnessRemote(send: .succeed))
        )
        ...
        let noneEngine = HarnessEngine()
        let noneGate = HarnessEngineGate()
        noneEngine.onExecute = { [noneGate] _ in noneGate.enter() }
        let noneStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: noneEngine, remote: HarnessRemote(send: .succeed))
        )
        ...
        let duplicateGate = HarnessEngineGate()
        duplicateEngine.onExecute = { [duplicateGate] _ in duplicateGate.enter() }
        let duplicateStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: duplicateEngine, remote: HarnessRemote(send: .succeed))
        )
        ...
        cancelEngine.onExecute = { [cancelGate] _ in cancelGate.enter() }
        let cancelStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: cancelEngine, remote: HarnessRemote(send: .succeed))
        )
        ...
        let staleGate = HarnessEngineGate()
        staleEngine.onExecute = { [staleGate] _ in staleGate.enter() }
        let staleStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(
                engine: staleEngine,
                remote: HarnessRemote(send: .succeed)
        ...
        let acceptedLocalMacEngine = HarnessEngine(executeResult: .ok)
        let acceptedLocalMacStore = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(
                engine: acceptedLocalMacEngine,
                remote: HarnessRemote(send: .succeed)
            )
        )
    ...
    func resumeAfterDeactivationUsesTheEnginesStickyPosition() async {
        ...
        let player = HarnessEnvironment.makePlaybackStore(
            HarnessEnvironment.make(engine: engine, remote: HarnessRemote(send: .succeed))
        )
        _ = player.send(.session(.ready), source: .account)
        _ = player.send(
            .devices(
