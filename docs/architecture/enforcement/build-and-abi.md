...
| `CI-REL-001` | Linux source policies, Rust, published-artifact Swift/architecture and Release lanes, plus candidate Debug/Release lanes (when engine inputs differ from the pinned release tag), feed the aggregate; noncanonical or unversioned playback pins are rejected | Workflow lane wiring is asserted from `Scripts/check.sh`; pin rejection and tag/input comparison are implemented in `.github/workflows/ci.yml` and require semantic workflow review; `test_playback_promotion.py` in the Rust/full gate covers promotion origin, job results, artifact integrity, provenance, and engine-version tag syntax |
| `CI-TOOL-001` | CI lanes select their documented toolchains and Linux/macOS runners, check out without persisted credentials, and restore caches by the exact content keys | literal workflow fragments asserted from `Scripts/check.sh`; the pin values are owned by the development setup guide and change together |
