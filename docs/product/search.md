...
- The short Songs preview uses its artwork for Play/Pause, following shared
  [playback intent](catalog-interaction.md#playback-intent) and
  [native focus/accessibility](catalog-interaction.md#focus-and-accessibility).
...
- Retrying the same completed query keeps its rows, selection, and scroll while fetching replacements.
  ...
  follows the shared [loading contract](catalog-interaction.md#loading-and-retained-content), with
  an error and retry in every category.
